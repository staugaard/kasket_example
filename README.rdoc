...
Because we have kasket on post_id on Comment, out code looks for the comments in the cache, then the DB.
...
We can see that kasket stores the individual comments in the cache an the a cache entry about the comments with post_id 1.
